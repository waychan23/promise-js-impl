...
function test(impl){
	...
	describe('#catch', function(){
	    it('[sync]should catch the thrown error in executor and invoke the catch()\'s onRejected callback', function(done){
		let promise = new JSPromise((resolve, reject) => {
		    throw new Error('manually thrown error');
		});
		promise.catch((reason) => {
		    assert.ok(reason instanceof Error);
		    assert.equal(reason.message, 'manually thrown error');
		    done();
		});
	    });

	    it('[sync]should catch the thrown error in executor and invoke the then()\'s onRejected callback', function(done){
		let promise = new JSPromise((resolve, reject) => {
		    throw new Error('manually thrown error');
		});
		promise.then(() => {
		    done(new Error('the onRejected callback should be invoked'));
		}, (reason) => {
		    assert.ok(reason instanceof Error);
		    assert.equal(reason.message, 'manually thrown error');
		    done();
	    ...
	    it('[sync]should catch the thrown error(string) in executor and invoke the catch()\'s onRejected callback', function(done){
		let promise = new JSPromise((resolve, reject) => {
		    throw 'manually thrown error';
		});
		promise.catch((reason) => {
		    assert.equal(reason, 'manually thrown error');
		    done();
		});
	    });

	    it('[sync]should catch the thrown error(string) in then() and invoke the catch()\'s onRejected callback', function(done){
		let promise = new JSPromise((resolve, reject) => {
		    resolve();
		});
		promise.then(() => {
	    ...
	    it('[sync]should catch the thrown error(string) in then()\'s onRejected() and invoke the then()\'s onRejected callback', function(done){
		let promise = new JSPromise((resolve, reject) => {
		    reject('reject reason');
		});
		promise.then(() => {
		    done(new Error('should invoke onRejected()'));
		}, (reason) => {
		    assert.equal(reason, 'reject reason');
		    throw 'thrown by then()\'s onRejected callback';
		}).catch((reason) => {
		    assert.equal(reason, 'thrown by then()\'s onRejected callback');
		    done();
		});
	    });
